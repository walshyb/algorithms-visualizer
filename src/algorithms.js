...
function findWasmBinary() {
  ...
  return new URL('algorithms.wasm', import.meta.url).href;
}
...
var _depth_first_search_tree = Module['_depth_first_search_tree'] = createExportWrapper('depth_first_search_tree', 5);
...
(() => {
  // Create a small, never-async wrapper around AlgorithmsModule which
  // checks for callers incorrectly using it with `new`.
  var real_AlgorithmsModule = AlgorithmsModule;
  AlgorithmsModule = function(arg) {
    if (new.target) throw new Error("AlgorithmsModule() should not be called with `new AlgorithmsModule()`");
    return real_AlgorithmsModule(arg);
  }
})();
